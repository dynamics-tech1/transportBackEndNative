...
class CommissionService {
  ...
  async createCommission({
    ...
    this.validateCommissionData({ commissionAmount });
    ...
    return this.executeInTransaction(async (connection) => {
      ...
      // 3. Verify commission rate exists
      ...
      // 4. Verify commission Status Exists
      if (!commissionStatusUniqueId) {
        throw new AppError("Commission Status Unique Id is required", 400);
      }
      ...
      // Log business event
      logger.application.commissionCreated(
        commissionData[0],
        commissionCreatedBy,
      );
  ...
  async getAllCommissions(filters = {}) {
    ...
    try {
      ...
      // Safe sorting
      const sortField =
        this.allowedSortFields[filters.sortBy] ||
        this.allowedSortFields.commissionId;
      const sortOrder =
  ...
  async updateCommission(id, data, updatedBy) {
    const fields = [];
    const values = [];
  ...
  async deleteCommission(id, deletedBy) {
    const sql = `
      UPDATE Commission 
